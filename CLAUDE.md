...
- **lw-gen base = RealVisXL V5.0 (ADR-010, 2026-08-16) - do not re-litigate on by-eye evidence.** Flipped back from Animagine XL 4.0 on a matched-seed three-base A/B with the adapter OFF: animagine sits **0.153 BELOW** the 0.8373 real-corpus self-similarity ceiling while still clearing the 0.26 subject floor, RealVis sits +0.024 ABOVE it and also wins subject_cos / margin / pass rate; DreamShaper XL clears it too (+0.008) at a sharpness cost. The ceiling is reproducible now - `tools/lw_gen_medium.py` re-derives it to four decimals - and it is a MEASURE, never a gate (one champion). Pair the base with the natural-language `splash` style; `splash-booru` is animagine's register and animagine stays on disk behind `--model-path`. UNTESTED by this flip: the Vayne glasses case that caused the 2026-07-11 move away from RealVis. Evidence: `docs/GEN_BASE_DECISION_2026-08-16.md`, LEDGER 114.
