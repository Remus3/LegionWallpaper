...
- Run the cheap local gate on the touched surface:
  ...
  - Test slice covering the change: full `tests/` for broad edits, the targeted module for narrow ones. (Product-specific suites: TBD - product not yet defined; add them here when the LW engine exists.)
